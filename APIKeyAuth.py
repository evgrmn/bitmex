...
def generate_signature(secret, verb, url, nonce, data):
    ...
    try:
        parsedURL = urlparse(url)			#python2
    except:
        parsedURL = urlparse.urlparse(url)			#python3
    path = parsedURL.path
    if parsedURL.query:
    ...
    try:
        signature = hmac.new(bytes(secret), bytes(message), digestmod=hashlib.sha256).hexdigest()		#python2
    except:
        signature = hmac.new(bytes(secret, 'utf8'), bytes(message, 'utf8'), digestmod=hashlib.sha256).hexdigest()		#python3
    return signature
